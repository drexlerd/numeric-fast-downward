...
class Action(object):
    ...
    def __repr__(self):
        return "<Action %s %s>" % (self.name, [str(x) for x in self.parameters])
        ...
        if effect_list:
            ...
            if task.metric[1] != -1:
                if self.cost is None:
                    # With NFD the cost field stores an evaluation of the
...
class PropositionalAction:
    def __init__(self, name, precondition, effects, cost):
        ...
        for (condition, effect) in effects:
            if not isinstance(effect, f_expression.FunctionAssignment) and effect.negated and \
                              (condition, effect.negate()) not in self.add_effects:
